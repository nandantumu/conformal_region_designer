...
def sliding_windows_stack_pred(data, prediction_future_window, skip_steps):
    ...
    for i in range(len(data)):
        ...
        _y = []
        for j in range(
            0, len(data[i]) - prediction_future_window * skip_steps, skip_steps
        ):
            _x.append(data[i][j])
            __y = []
            for k in range(prediction_future_window):
                for data_type_ind in range(
                    len(data[i][j + skip_steps + k * skip_steps])
                ):  ## pattern is [type_1_time_0,type_2_time_0,type_1_time_1,type_2_time_1,...]
                    __y.append(
                        data[i][j + skip_steps + k * skip_steps][data_type_ind]
                    )  ## SEEME: should the [0] be [:] instead?
            _y.append(__y)
        x.append(_x)
        y.append(_y)
...
def main():
    ...
    for i in range(train_size):
        for j in range(len(all_data[input_types[0]][i])):
            all_data_one_array.append(
                [all_data[input_type][i][j] for input_type in input_types]
            )

    train_data = np.array(
        [
            [all_data[input_types[j]][i] for j in range(len(input_types))]
            for i in range(train_size)
        ]
    )
    ...
    trace_data = [
        [
            sc.transform([[all_data[input_type][i][j] for input_type in input_types]])[
                0
            ]
            for j in range(len(all_data[input_types[0]][i]))
        ]
        for i in range(len(all_data[input_types[0]]))
    ]
    ...
    skip_steps = 3  # 5
    x, y = sliding_windows_stack_pred(
        trace_data, prediction_future_window, skip_steps
    )  ## TODO: theres a bug in here
    ...
    if save_train_test:
        ...
        data_folder = (
            "data/stackedOutput/3400DataPoints/pred_future_window_"
            + str(prediction_future_window)
            + "/skip_step_"
            + str(skip_steps)
        )
    ...
    model_folder = (
        "models/stackedOutput/3400DataPoints/pred_future_window_"
        + str(prediction_future_window)
        + "/skip_step_"
        + str(skip_steps)
    )
    ...
    for j in range(trials_to_plot):
        ...
        _sample_output_plot = (
            sample_output.view(sample_data.size(1), -1).cpu().data.numpy()
        )
        ...
        for i in range(_sample_predict_plot.shape[0]):
            ...
            for k in range(0, len(curr_pred), 2):
                temp_preds.append(
                    [curr_pred[k], curr_pred[k + 1]]
                )  ## SEEME: this is now hardcoded for 2 outputs
        ...
        resid = [
            pred_of_interest[0] - gt_of_interest[0],
            pred_of_interest[1] - gt_of_interest[1],
        ]
    ...
    image_dir = (
        "images/stackedOutput/3400DataPoints/pred_future_window_"
        + str(prediction_future_window)
        + "/skip_step_"
        + str(skip_steps)
        + "/alt_theta/"
