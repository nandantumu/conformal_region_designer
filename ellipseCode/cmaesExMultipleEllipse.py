...
def computeCPEllipse(x_vals,y_vals,x_hats,y_hats,a,b,c,delta):
    R_vals = [ a*(x_vals[i]-x_hats[i])**2 + 2*b*(x_vals[i]-x_hats[i])*(y_vals[i]-y_hats[i]) + c*(y_vals[i]-y_hats[i])**2 for i in range(len(x_vals))]
...
def computeCoverageCircle(x_vals,y_vals,x_hats,y_hats,Ds_cp):
    ...
    coverage_count = sum([1 if math.sqrt((x_vals[j]-x_hats[j])**2 + (y_vals[j]-y_hats[j])**2) < Ds_cp else 0 for j in range(len(x_vals))])
...
def objection_func_cma_es_matrix_ellipse(x,*args):
    ...
    a_rotated = -1/(b**2/4-4*a*c)*math.sqrt(2*(b**2/4-4*a*c)*(-D_cp)*((a+c)+math.sqrt((a-c)**2+b**2/4)))
    c_rotated = -1/(b**2/4-4*a*c)*math.sqrt(2*(b**2/4-4*a*c)*(-D_cp)*((a+c)-math.sqrt((a-c)**2+b**2/4)))
...
def computeCPEllipseMatrix(gt_vals,pred_vals,Q,delta):
    ...
    R_vals = [np.matmul(np.matmul(gt_vals[i]-pred_vals[i],Q),gt_vals[i]-pred_vals[i]) for i in range(gt_vals.shape[0])]
...
def computeCPEllipseMatrixManyEllipse(residuals,Q_matrices,ellipse_centers,delta):
    ...
    R_vals = [min([np.matmul(np.matmul(residuals[i]-ellipse_centers[j],Q_matrices[j]),residuals[i]-ellipse_centers[j]) for j in range(len(Q_matrices))]) for i in range(residuals.shape[0])]
...
def callCMAESMatrixManyEllipse(residuals,delta,num_ellipse,ellipse_centers=None):
    ...
    start_time = time.time()
    x,es = cma.fmin2(objective_func_cma_es_ellipse_arbitrary_dim_multi_ellipse,x_0,sigma0,args=args_cma)
    end_time = time.time()
    ...
    for i in range(num_ellipse):
        ...
        x_temp = x_temp.reshape((int(math.sqrt(x_temp.size)),int(math.sqrt(x_temp.size))))
        if not pre_defined_centers:
            center_temp = x[i*num_ellipse_params+dims**2:(i+1)*num_ellipse_params]
...
def objective_func_cma_es_ellipse_arbitrary_dim_multi_ellipse(x,*args):
    ...
    if args[4] is None:
        ...
        num_ellipse_params = dims**2 + dims # first term for the matrix, second for the offset
        pre_defined_centers = False
    else:
    ...
    for i in range(num_ellipse):
        ...
        if np.linalg.matrix_rank(Q) != Q.shape[0]: ## Q not full rank
            return np.NaN
        if not (np.allclose(Q,Q.T,rtol=1e-05,atol=1e-08)): ## Q not symmetric (should not be possible for this to happen given how Q is constructed)
            return np.NaN
        Q_matrices.append(Q)
        

    # D_cp = computeCPEllipseMatrix(gt_vals,pred_vals,Q,delta)
    D_cp = computeCPEllipseMatrixManyEllipse(residuals,Q_matrices,ellipse_centers,delta)
    ...
    vol = 0
    for i in range(num_ellipse):
        vol += 1/math.sqrt(np.linalg.det(Q_matrices[i]/D_cp)) ## This should be scaled by the volume of the unit ball, but that's constant so we don't need to use it
...
def sample_circle_unif(center,radius,num_points,x_stretch=1,y_stretch=1):
    ...
    for _ in range(num_points):
        ...
        point = [center[0]+rad*x_stretch*math.cos(angle),center[1]+rad*y_stretch*math.sin(angle)]
...
def plot_ellipse(center,Q,D_cp,color,label):
    ...
    ellipsis = (1/np.sqrt(eigenvalues[None,:]/D_cp) * eigenvectors) @ [np.sin(theta), np.cos(theta)]
...
def main_fake_initial_example():
    ...
    num_samples = 100
    samples_circle_1 = sample_circle_unif(circle_1_center,circle_1_radius,num_samples,x_stretch=2)
    ...
    Q_matrices,ellipse_centers = callCMAESMatrixManyEllipse(samples,delta,num_ellipse,ellipse_centers=ellipse_centers)
    ...
    D_cp = computeCPEllipseMatrixManyEllipse(samples,Q_matrices,ellipse_centers,delta)
    print("D_cp: " + str(D_cp))
