...
class LSTMNew(nn.Module):
    ...
    def __init__(self, num_classes, input_size,lstm_hidden_size,hidden_size, lstm_num_layers,device):
    ...
    def forward(self, x,h_0=None,c_0=None):
        if h_0 is None:
            h_0 = Variable(torch.zeros(
                self.num_layers, x.size(0), self.lstm_hidden_size)).to(self.device)
        if c_0 is None:
            c_0 = Variable(torch.zeros(
                self.num_layers, x.size(0), self.lstm_hidden_size)).to(self.device)
